...
const StyledButton = styled.button<{ $variant: string; $size: string , $transparent: boolean}>`
  ...
  color: ${$transparent ? theme?.palette?.[$variant]?.[500] : theme?.palette?.neutral?.[0]};
  background: ${$transparent ? "transparent" : theme?.palette?.[$variant]?.[500]};
  border: ${$transparent ? `1px solid ${theme?.palette?.[$variant]?.[500]}` : "0"};
`};`;
